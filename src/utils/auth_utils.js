...
var auth = {
    ...
    getObject: (main_key, sub_key)=> auth.data[main_key] ? auth.data[main_key][sub_key] : null,
    ...
    getDorm: ()=> (auth.getDormBulid()||'')+'-'+(auth.getDormRoom()||''),
    ...
    update: async (id) => {
        ...
        auth.type = data.type; auth.token = data.token;
    ...
    get: async (id) => {
        ...
        if(id) return res; auth.setup(res);
    ...
    // 编辑用户
    put: async (id, department_id, group_id) => (await auth.oper(
        'PUT', id, {group_id, department_id})),
    // 回收用户
    ...
    create: async (snos, department, group) => {
        ...
        var res = await auth.send("POST",
            CreateRoute, {data, group, department});
    ...
    setup: (data) => {
        if(!data || data.error) auth.clear();
            ...
            if(key === 'detail') for(let dkey in data[key])
    ...
    login: async (username, password) => {
        let data = await auth.send("POST", LoginRoute, {sno: username, password});
        auth.type = data.type; auth.expires_in = data.expires_in; auth.token = data.token;
    ...
    resetCode: async (username, email) => {
        let data = await auth.send("POST", ResetCodeRoute, {sno: username, email}) || {};
        return !data.error;
    },

    // 重置密码
    reset: async (username, password, code) => {
        let data = await auth.send("POST", ResetRoute, {sno: username, captcha: code, password}) || {};
        return !data.error;
    },

    // 修改密码
    editPwd: async (old_password, password, password2) => {
        let data = await auth.send("PUT", ResetRoute2, {old_password, password, password2}) || {};
        return !(data && data.error);
    },

    // 设置邮箱验证码
    emailCode: async (email) => {
        let data = await auth.send("POST", SetEmailCodeRoute, {email}) || {};
        return !data.error;
    },

    // 设置邮箱
    setEmail: async (code) => {
        let data = await auth.send("POST", SetEmailRoute, {captcha: code}) || {};
        return !data.error;
    },
