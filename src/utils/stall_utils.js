...
var stall = {
    ...
    deleteStall: async (id) => !(await stall.oper("DELETE", DeleteStallRoute, id)),
    ...
    getSubStallMember: async (id)=> await stall.oper("GET", GetSubStallMemberRoute, id),

    addSubStallDuty: async (id, sno)=> await stall.oper("POST", AddDutySubStallRoute, id, {sno}),

    deleteSubStallDuty: async (id)=> await stall.oper("DELETE", DeleteDutySubStallRoute, id),

    deleteSubStall: async (id)=> await stall.oper("DELETE", DeleteSubStallRoute, id),

    createSubStall: async (stall_id, location, date, number, start, end) =>
        await stall.oper("POST", CreateSubStallRoute, undefined,
            {stall_id, location, date, number, start, end}),
