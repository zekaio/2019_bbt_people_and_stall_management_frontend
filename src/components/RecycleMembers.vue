<template>
    <div>
                    ...
                    <el-table :data="page_data" height="100%" stripe border>
                        ...
                        <el-table-column label="操作" fixed="right">
                            <template slot-scope="scope">
                                ...
                                <el-button @click="onRecycle(scope.row)" type="text">回收</el-button>
        ...
        <transition name="el-zoom-in-center">
            <el-collapse id="detail_content" v-model="active_tags" v-show="detail">
...
<script>
    ...
    async function refreshDetail(id) {
        ...
        if(member) this.detail_data = getDetailData();
        return !!member;
    }
    ...
    async function onRecycleConfirm(row) {
        let res = await auth.recy(row.id);
        if(res) {
            loadData(this); web.web.showAlert(row.name+" 已回收！");
        }
    }
    async function onRecycle(row) {
        web.web.showConfirm("确认恢复已删除的成员 "+row.name+" ？", onRecycleConfirm.bind(this, row));
    }
    ...
    async function onDeleteConfirm(row) {
        ...
        if(res) {
            loadData(this); web.web.showAlert(row.name+" 已彻底删除！");
        }
    }
    */

    async function onDelete(row) {
        web.web.showAlert("暂不提供此功能！");
        //web.web.showConfirm("确认彻底删除 "+row.name+" ？", onDeleteConfirm.bind(this, row));
    }

    async function loadData(self) {
        let res = await admin.getRecycleMembers(self.current_page);
        if(res){
    ...
    export default {
        data() {
            web.web.checkLogin(this) && loadData(this);
            return {
                active_tags: ['1'],
                current_page: 1,
                per_page: 20, total: 0,
...
<style scoped>
    ...
    #edit_content {
        ...
        min-width: 444px; max-width: 540px;
        text-align: center;
        border-radius: 4px;
        background: rgba(233,233,233,0.8);
    }
