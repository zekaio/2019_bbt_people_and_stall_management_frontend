    ...
    <div>
            ...
            <div id="main_content" v-show="!detail && !edit">
                ...
                >
                    ...
                    <el-row
                        :gutter="20"
                        class="grid-content"
                        v-for="(item, index) in stall.detail"
                        :key="index"
                    >
                        <el-col :span="8"
                            ><div>
                ...
                <el-row :gutter="20" class="edit-grid-content bg-purple">
                    ...
                    <el-col :span="20"
                        ><div id="desc_container">
                            <el-input
                                id="desc_input"
                                v-model="selected_stall.description"
                                type="textarea"
                                :rows="8"
                                clearable
                                @focus="onFocus"
        ...
        <transition name="el-zoom-in-center">
            <el-collapse
                id="detail_content"
                v-model="active_tags"
                v-show="detail && !add_substall && !sub_stall_detail"
            >
                <el-collapse-item title="基本信息" name="1">
                    <el-row :gutter="20" class="detail-grid-content">
                        <el-col :span="8"
                            ><div><b>项目名称</b></div></el-col
                        >
                        <el-col :span="16"
                            ><div>{{ selected_stall.name }}</div></el-col
                        >
                    </el-row>
                    <el-row
                        :gutter="20"
                        class="detail-grid-content"
                        v-for="(item, index) in selected_stall.detail"
                        :key="index"
                    >
                        <el-col :span="8"
                            ><div>
        ...
        <transition name="el-zoom-in-center">
            <div id="add_sub_stall_content" v-show="add_substall">
                <h1>
                    为 {{ selected_stall.name }} 添加第
                    {{ sub_stalls.length + 1 }} 个摆摊内容
                </h1>
                ...
                <el-row :gutter="20" class="detail-grid-content bg-purple">
                    ...
                    <el-col :span="20"
                        ><div>
                            <el-select
                                id="campus"
                                v-model="create.campus"
                                placeholder="请选择"
                            >
                                <el-option
                                    v-for="opt in create.campus_options"
                                    :key="opt"
                                    :label="opt"
                                    :value="opt"
                                >
                ...
                <el-row :gutter="20" class="detail-grid-content bg-purple">
                    ...
                    <el-col :span="20"
                        ><div>
                            <el-select
                                id="start_sess"
                                v-model="create.start_sess"
                                placeholder="请选择"
                            >
                                <el-option
                                    v-for="opt in create.sess_options"
                                    :key="opt"
                                    :label="opt"
                                    :value="opt"
                                >
                ...
                <el-row :gutter="20" class="button_content">
                    <el-button id="add" @click="addSubStall" type="primary"
                        >添加</el-button
                    >
                    <el-button id="close" @click="closeSubStall" type="primary"
                        >返回</el-button
                    >
                </el-row>
            </div>
        ...
        <transition name="el-zoom-in-center">
            <el-collapse
                id="sub_stall_detail_content"
                v-model="active_tags"
                v-show="sub_stall_detail"
            >
                <el-collapse-item title="摆摊信息" name="1">
                    <el-row
                        :gutter="20"
                        class="detail-grid-content"
                        v-for="(item, index) in selected_sub_stall_detail"
                        :key="index"
                    >
                        <el-col :span="8"
                            ><div>
                ...
                <el-collapse-item title="人员信息" name="2">
                    ...
                    <el-row
                        :gutter="20"
                        class="detail-grid-content button_content"
                        v-if="!no_duty"
                    >
                        <el-button
                            id="random_member"
                    ...
                    <div class="table_content">
                        <el-table :data="member_data" stripe border>
                            ...
                            >
                                <template slot-scope="scope">
                                    <el-button
                                        @click="onMemberCheckIn(scope.row)"
                                        type="text"
                                        >签到</el-button
                                    >
                                    <el-button
                                        @click="onMemberDelete(scope.row)"
                                        type="text"
                                        v-if="member_data.length <= 1"
                                        >删除</el-button
                                    >
                                </template>
                            </el-table-column>
                ...
                <el-row :gutter="20" class="button_content">
                    <el-button
                        id="close_sub_stall_detail"
                        @click="sub_stall_detail = false"
                        type="primary"
                        >关闭</el-button
                    >
                </el-row>
...
function makeDetail(stall) {
    return [
        ...
        {
            field: "发起部门",
            value: data.getDeptById(stall.department_id).name
        },
        { field: "项目描述", value: stall.description },
        {
            field: "任务数量",
            value: stall.stall_tasks[0] ? stall.stall_tasks[0].tasks_number : 0
        }
    ];
}
...
async function onEditStallConfirm() {
    ...
    let res =
        ...
        (await stall.updateStall(
            stall_data.id,
            stall_data.name,
            stall_data.description,
            auth.getDeptId()
        ));
    if (res) {
...
function deleteStall(stall) {
    web.web.showConfirm(
        "确定删除项目 " + stall.name + " 吗？",
        onDeleteConfirm.bind(this, stall)
    );
}
...
function validateSubStall() {
    ...
    if (!this.create.date) flag = web.web.setInvalid(date_input, DateEmpty);
    return flag;
}
...
async function addSubStall() {
    ...
    res = await stall.createSubStall(
        this.selected_stall.id,
        this.create.campus,
        this.create.location,
        date,
        this.create.num,
        this.create.start_sess,
        this.create.end_sess
    );
    console.info(res);
    if (res && !res.error) {
        web.web.showAlert(
            "添加摆摊成功！目前本项目已有 " +
                (this.sub_stalls.length + 1) +
                " 个摆摊活动，你可继续添加。"
        );
...
function makeMemberData(members) {
    let res = [];
    for (var i in members) res.push(convertToMemberData(members[i]));
    return res;
}
...
async function onDelete(row) {
    web.web.showConfirm(
        "真的要删除该摆摊吗？",
        onDeleteSubStallConfirm.bind(this, row)
    );
}
...
async function onDeleteMemberConfirm(row) {
    ...
    if (row.role === "是") {
        ...
        if (!res) {
            ...
            if ((res = await stall.getSubStallMember(id))) {
                this.member_data = makeMemberData(res);
                this.no_duty = !this.member_data.some(
                    data => data.role === "是"
                );
            }
        }
    } else web.web.showAlert("暂不支持删除普通成员！");
}

function onMemberDelete(row) {
    web.web.showConfirm(
        "真的要删除 " + row.name + " 吗？",
        onDeleteMemberConfirm.bind(this, row)
    );
}
