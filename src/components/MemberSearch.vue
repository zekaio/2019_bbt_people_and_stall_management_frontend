    ...
    <div>
        <transition name="el-zoom-in-center">
            <el-collapse id="main_content" v-model="active_tags" v-show="!detail && !edit">
                <el-collapse-item class="item" title="快速查询" name="1">
                    <div class="tips">注：支持通过 姓名、专业、手机号、短号、QQ、微博、管理层、部门 模糊查询</div>
                    <el-row :gutter="20" class="grid-content">
                        <el-col :span="4"><b>关键字</b></el-col>
                        <el-col :span="15"><div>
                            <el-input id="keyword" v-model="keyword" clearable @focus="onFocus"
                                      placeholder="输入关键字">
                            </el-input>
                        </div></el-col>
                        <el-col :span="5" id="query_button">
...
<script>
    ...
    const KeywordEmpty = "查询项不可为空！";

    function validateKeyword() {
        let flag = true;
        if(keyword.value.length <= 0) flag = web.web.setInvalid(keyword, KeywordEmpty);
        return flag;
    }
    ...
    async function loadData(self) {
        let res = await admin.searchMembers(self.keyword, self.current_page);
        if(res){
